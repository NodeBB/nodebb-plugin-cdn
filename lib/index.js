...
exports.preLoad = ({ app }, callback) => {
  ...
  getObject('plugin_cdn:settings', (err, settings) => {
    ...
    console.log(`RELPATH is ${relativePath} and settings is ${JSON.stringify(settings)}`);

    if (settings && settings.enabled) {
      console.log(`USEPATH is ${relativePath}/assets`);
      app.use(`${relativePath}/assets`, (req, res) => {
        console.log(`REDIRPATH is ${settings.url}/${req.path}?${meta.config['cache-buster']}`);
